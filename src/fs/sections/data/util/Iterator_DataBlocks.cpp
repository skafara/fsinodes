...
Iterator_DataBlocks &Iterator_DataBlocks::operator++() {
	...
	if (it_indirect1) {
		...
		if (it_direct_no >= kBlock_Refs_Cnt) {
			it_indirect1 = false;
			it_indirect2 = true;
			it_indirect1_no = 0;
			it_direct_no = 0;
			...
			_direct_ref = _data->Get(_direct_ref).Read_Reference(it_direct_no);
			if (_direct_ref == Inode::kIndirect_Ref_Unset) {
				Is_Depleted = true;
				return *this;
			}
	...
	if (it_indirect2) {
		...
		if (it_indirect1_no >= kBlock_Refs_Cnt) {
			Is_Depleted = true;
			return *this;
		}

		const uint32_t dblock_indirect1_idx = _data->Get(_inode.Get_Indirect2()).Read_Reference(it_indirect1_no);
		if (dblock_indirect1_idx == Inode::kIndirect_Ref_Unset) {
			Is_Depleted = true;
			return *this;
		}

		_direct_ref = _data->Get(dblock_indirect1_idx).Read_Reference(it_direct_no);
		if (_direct_ref == Inode::kIndirect_Ref_Unset) {
			Is_Depleted = true;
			return *this;
		}
...
Iterator_DataBlocks &Iterator_DataBlocks::Append(const t_DataBlockAcquirer& acquirer) {
	...
	if (it_indirect1) {
		...
		_data->Get(_inode.Get_Indirect1()).Write_Reference(it_direct_no, _direct_ref);
	}
	if (it_indirect2) {
		...
		if (dblock_indirect2.Read_Reference(it_indirect1_no) == Inode::kIndirect_Ref_Unset) {
			dblock_indirect2.Write_Reference(it_indirect1_no, _direct_ref);
			_direct_ref = acquirer();
		}

		DataBlock dblock_indirect1 = _data->Get(dblock_indirect2.Read_Reference(it_indirect1_no));
		dblock_indirect1.Write_Reference(it_direct_no, _direct_ref);
	}
...
void Iterator_DataBlocks::Release_All(Inode &inode, const std::shared_ptr<Data> &data, const t_DataBlockReleaser &releaser) {
	if (inode.Get_Indirect2() != Inode::kIndirect_Ref_Unset) {
		...
		for (uint32_t i = 0; i < kBlock_Refs_Cnt; ++i) {
			const uint32_t indirect1 = dblock_indirect2.Read_Reference(i);
			if (indirect1 == Inode::kIndirect_Ref_Unset) {
				break;
			}

			DataBlock dblock_indirect1 = data->Get(indirect1);
			for (uint32_t j = 0; j < kBlock_Refs_Cnt; ++j) {
				const uint32_t direct = dblock_indirect1.Read_Reference(j);
	...
	if (inode.Get_Indirect1() != Inode::kIndirect_Ref_Unset) {
		...
		for (uint32_t i = 0; i < kBlock_Refs_Cnt; ++i) {
			const uint32_t direct = dblock_indirect1.Read_Reference(i);
			if (direct == Inode::kIndirect_Ref_Unset) {
				break;
			}
